...
if [ -f ${PREFIX}.pair.uniq.split.fastq ] && [ -f ${PREFIX}.pair.uniq.split.bed ];then
	...
	bedtools bamtobed -i ${PREFIX}.pair.uniq.split.bam -tag NM | awk -v div=${MISMATCH} 'BEGIN{FS=OFS="\t"} {$4=substr($4,1,length($4)-2)"_"substr($4,length($4));if($5<=(int(($3-$2-1)*div/100)+1)){if($6=="+"){$6="-"}else{$6="+"};print $0}}' - > ${PREFIX}.pair.uniq.split.bed
	if [ ${ALU_MODE} ];then
...
if [ -z ${FREQ_CUTOFF} ];then
	...
	intersectBed -b ${PREFIX}.tmp.bed -a ${PREFIX}.TPregion.bed -wo -s -F 1 | awk 'BEGIN{FS=OFS="\t"} {n=$1;if(ARGIND==1){a[n]+=$11}else{$2=a[n];$6=a[n]-$8;$7=a[n]-$9;if($6<0){$6=0};if($7<0){$7=0};print $0}}' - ${PREFIX}.soma.rate.bed > ${PREFIX}.tmp
