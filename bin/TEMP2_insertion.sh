...
awk 'BEGIN{FS=OFS="\t"} {if($7=="singleton"){split($4,a,",");for(i in a){split(a[i],b,":");if($10>0 && b[4]=="+"){st="-"}else if($10>0 && b[4]=="-"){st="+"}else if(b[4]=="+"){st="+"}else{st="-"};print b[1],b[2],b[3],0,$8/length(a),st}}}' ${PREFIX}.insertion.bed > ${PREFIX}.tmp.bed
